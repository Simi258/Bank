    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto border-indigo-400 sm:px-6 lg:px-8">
            <div class="bg-white border  border-indigo-400  shadow-md rounded-md p-8  max-w-xl mx-auto">
                ...
                <form method="POST" action="{{ route('admin.user-register-account', ['id' => $accounts->id]) }}">
                    @csrf
                    <div class="mb-6">
                        <x-input-label for="iban" :value="__('IBAN')" />
                        <x-text-input id="iban" class="block w-full mt-1" type="text" name="iban" :value="old('iban')" required autofocus autocomplete="iban" />
                        <x-input-error :messages="$errors->get('iban')" class="mt-2" />
                    </div>
                    <div class="mb-6">
                        <x-input-label for="pin" :value="__('Pin')" />
                        <x-text-input id="pin" class="block w-full mt-1" type="text" name="pin" :value="old('pin')" required autofocus autocomplete="pin" />
